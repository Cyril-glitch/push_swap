...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/20 14:10:05 by cycolonn          #+#    #+#             */
/*   Updated: 2026/01/20 14:15:36 by cycolonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_printlst(t_clist *stack_a, t_clist *stack_b)
{
	...
	while (i < size)
	{
		if (i < size_a)
		{
			...
			res = ft_printf("[%d] = %d [targ = %d][cost = %d][strat = %d]",
							current_a->index,
							current_a->content,
							target_a,
							current_a->cost,
		...
		if (i < size_b)
		{
			...
			ft_printf("[%d] = %d [targ = %d][cost = %d][strat = %d]",
						current_b->index,
						current_b->content,
						target_b,
						current_b->cost,
